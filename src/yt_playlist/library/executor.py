...
# The quarantine marker, written into the playlist's DESCRIPTION on YouTube.
#
# Which group a playlist belongs to is what keeps a generated playlist out of the taste model, and
# it used to live only in the local database - so a second machine, or a reinstall, synced these
# back as ordinary playlists and started reading the app's own suggestions as things you chose. The
# YouTube account is the one thing every install shares, so the marker belongs there. Sync reads it
# back (library/sync.py) and promotion rewrites it (routes/playlists.py), which makes promoting on
# one machine visible on all of them.
GENERATED_DESCRIPTION = "Generated by TuneConsole"
PROMOTED_DESCRIPTION = "Promoted into your library from TuneConsole"


def is_generated_description(text) -> bool:
    """Does this playlist description mark it as one the app generated (and not yet promoted)?"""
    return GENERATED_DESCRIPTION.lower() in (text or "").strip().lower()
...
def create_generated_playlist(store, title, tracks, client, now, identity_id=None, group=None,
    ...
    new_pid = client.create_playlist(title, GENERATED_DESCRIPTION)
